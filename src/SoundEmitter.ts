...
export class SoundEmitter extends Entity {
    ...
    public constructor(
        scene: GameScene, position: Point, sound: Sound, maxVolume: number, intensity: number
    ) {
        ...
        if (soundId) {
            ...
            if (sound) {
                return new SoundEmitter(
                    scene, new Point(gameObjectInfo.position.x, gameObjectInfo.position.y), sound, volume, intensity
                );
            } else {
                throw new Error(
