...
export abstract class NPC extends PhysicsEntity {
    ...
    public speechBubble = new SpeechBubble(this.gameScene).appendTo(this);
    public lookAtPlayer = true;
    public dialoguePrompt = new DialoguePrompt();
    ...
    protected showDialoguePrompt(): boolean {
        if (this.hasActiveConversation() || !this.gameScene.player.isControllable) {
            return false;
        }
    ...
    public registerEndedConversation(): void {
        this.lastEndedConversation = this.gameScene.gameTime;
    }

    public isReadyForConversation(): boolean | null {
        return (
            this.conversation
            && !this.gameScene.player.isCarrying(this)
            && this.gameScene.gameTime - this.lastEndedConversation > PAUSE_AFTER_CONVERSATION
        );
    }

    public hasActiveConversation(): boolean {
        return (this.gameScene.player.playerConversation !== null && this.gameScene.player.playerConversation.npc === this);
    }
    ...
    public update(dt: number): void {
        if (this.lookAtPlayer) {
            const dx = this.gameScene.player.x - this.x;
